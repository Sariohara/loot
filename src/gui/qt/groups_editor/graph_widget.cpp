...
std::map<std::string, Node *>::iterator insertNode(
  ...
  auto node = new Node(graphWidget,
...
bool GraphWidget::addGroup(const std::string &name) {
  auto qName = QString::fromUtf8(name);
...
std::vector<Group> GraphWidget::getUserGroups() const {
...
void GraphWidget::handleGroupSelected(const QString &name) {
