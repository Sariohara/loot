...
#include "gui/qt/groups_editor/graph_widget.h"
...
Node::Node(GraphWidget *graphWidget,
  ...
  textItem->setPos(-textWidth / 2, TEXT_Y_POS);
}
...
QRectF Node::boundingRect() const {
  auto halfTextWidth = textItem->boundingRect().width() / 2;

  auto leftPos = std::min(halfTextWidth, double(-RADIUS));
  auto width = std::max(double(DIAMETER), halfTextWidth);
  auto height = RADIUS + TEXT_Y_POS + textItem->boundingRect().height();
...
void Node::mousePressEvent(QGraphicsSceneMouseEvent *event) {
  ...
  if (isUserMetadata_ && !containsInstalledPlugins &&
    ...
    auto graphicsWidget = qobject_cast<GraphWidget *>(scene()->parent());
...
QColor Node::getNodeColor() const {
  ...
  return QGuiApplication::palette().color(colorGroup, QPalette::Text);
}
