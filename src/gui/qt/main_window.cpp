  ...
  try {
    ...
    for (const auto& gameSettings : state.getGameSettings()) {
      ...
      if (installedGame != installedGames.cend()) {
        gameComboBox->addItem(QString::fromUtf8(gameSettings.Name()),
                              QString::fromUtf8(gameSettings.FolderName()));
      }
    }
    ...
    gameComboBox->setCurrentText(
        QString::fromUtf8(state.GetCurrentGame().Name()));
...
void MainWindow::setupMenuBar() {
  ...
  actionEditMetadata->setIcon(IconFactory::getEditMenuIcon());
...
void MainWindow::setupViews() {
  ...
  pluginCardsView->setSelectionMode(QAbstractItemView::SingleSelection);
...
void MainWindow::loadGame(bool isOnLOOTStartup) {
  ...
  auto sendProgressUpdate = [progressUpdater](std::string message) {
    emit progressUpdater->progressUpdate(QString::fromUtf8(message));
  };
...
void MainWindow::openPersistentEditors(int firstRow, int lastRow) {
  for (int row = firstRow; row <= lastRow; row += 1) {
    auto proxyIndex = proxyModel->index(row, PluginItemModel::CARDS_COLUMN);

    if (!pluginCardsView->isPersistentEditorOpen(proxyIndex)) {
      pluginCardsView->openPersistentEditor(proxyIndex);
    }
  }
}
...
void MainWindow::handleError(const std::string& message) {
  ...
  QMessageBox::critical(this, translate("Error"), QString::fromUtf8(message));
}
...
void MainWindow::on_actionSettings_triggered(bool checked) {
  try {
    ...
    settingsWindow->initialiseInputs(state, themes, currentGameFolder);
    settingsWindow->show();
...
void MainWindow::on_actionCopyMetadata_triggered(bool checked) {
  try {
    ...
    showNotification(QString::fromUtf8(text));
  } catch (std::exception& e) {
    handleException(e);
  }
}
...
void MainWindow::on_actionClearMetadata_triggered(bool checked) {
  try {
    ...
    showNotification(QString::fromUtf8(notificationText));
  } catch (std::exception& e) {
    handleException(e);
  }
}
...
<blockquote>
  ...
  You should have received a copy of the GNU General Public License along with LOOT. If not, see &lt;https://www.gnu.org/licenses/&gt;.
    ...
    QMessageBox::about(this, translate("About LOOT"), QString::fromUtf8(text));
  } catch (std::exception& e) {
    handleException(e);
  }
}
...
void MainWindow::on_proxyModel_dataChanged(const QModelIndex& topLeft,
                                           const QModelIndex& bottomRight,
                                           const QList<int>& roles) {
  if (!topLeft.isValid() || !bottomRight.isValid()) {
    return;
  ...
  if ((roles.isEmpty() || roles.contains(RawDataRole) ||
       roles.contains(CardContentFiltersRole)) &&
      (topLeft.column() == PluginItemModel::CARDS_COLUMN ||
       bottomRight.column() == PluginItemModel::CARDS_COLUMN)) {
    openPersistentEditors(topLeft.row(), bottomRight.row());
  }
}

void MainWindow::on_proxyModel_rowsInserted(const QModelIndex& parent,
                                            int first,
                                            int last) {
...
void MainWindow::on_settingsWindow_accepted() {
  try {
    settingsWindow->recordInputValues(state);
  } catch (std::exception& e) {
    handleException(e);
  }
}
