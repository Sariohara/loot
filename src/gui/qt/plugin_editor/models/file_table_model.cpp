...
QVariant FileTableModel::data(const QModelIndex& index, int role) const {
  ...
  switch (index.column()) {
    case 0:
      return QVariant(QString::fromUtf8(std::string(element.GetName())));
    case 1:
      return QVariant(QString::fromUtf8(element.GetDisplayName()));
    case 2: {
      if (role == Qt::DisplayRole) {
        auto contentText =
            element.ChooseDetail(language).value_or(MessageContent()).GetText();
        return QVariant(QString::fromUtf8(contentText));
      }
      return QVariant::fromValue(element.GetDetail());
    }
    case 3:
      return QVariant(QString::fromUtf8(element.GetCondition()));
    default:
      return QVariant();
  }
}
...
bool FileTableModel::dropMimeData(const QMimeData* data,
  ...
  setData(newIndex, QVariant(QString::fromUtf8(filename)), Qt::EditRole);
