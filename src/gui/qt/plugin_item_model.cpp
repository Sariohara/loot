    ...
    switch (index.column()) {
      case SIDEBAR_LOAD_ORDER_COLUMN: {
        if (role == Qt::DisplayRole) {
          return QString::fromUtf8(plugin.loadOrderIndexText());
        }
      ...
      case SIDEBAR_NAME_COLUMN: {
        if (role == EditorStateRole) {
          ...
          return QString::fromUtf8(plugin.name);
        }
      ...
      case SIDEBAR_STATE_COLUMN: {
        ...
        if (role == Qt::DecorationRole) {
          if (isCurrentEditorPlugin) {
            return IconFactory::getSidebarEditIcon();
          } else if (plugin.hasUserMetadata) {
            return IconFactory::getSidebarHasUserMetadataIcon();
          } else {
            return QVariant();
          }
      ...
      case CARDS_COLUMN: {
        if (role == CardContentFiltersRole) {
          ...
          int itemsIndex = index.row() - 1;
          return QString::fromUtf8(items.at(itemsIndex).contentToSearch());
...
QMimeData* PluginItemModel::mimeData(const QModelIndexList& indexes) const {
  ...
  for (const QModelIndex& index : indexes) {
    if (index.isValid() && index.row() > 0) {
      ...
      encodedData.append(text.toStdString());
    }
  }
