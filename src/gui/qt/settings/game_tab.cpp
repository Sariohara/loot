...
void GameTab::initialiseInputs(const GameSettings& settings,
                               bool isCurrentGame) {
  nameInput->setText(QString::fromUtf8(settings.Name()));
  lootFolderInput->setText(QString::fromUtf8(settings.FolderName()));
  masterFileInput->setText(QString::fromUtf8(settings.Master()));
  minimumHeaderVersionSpinBox->setValue(settings.MinimumHeaderVersion());
  repoUrlInput->setText(QString::fromUtf8(settings.RepoURL()));
  repoBranchInput->setText(QString::fromUtf8(settings.RepoBranch()));
  installPathInput->setText(QString::fromUtf8(settings.GamePath().u8string()));
  localDataPathInput->setText(
      QString::fromUtf8(settings.GameLocalPath().u8string()));
  ...
  for (const auto& gameType : GAME_TYPES_BY_FOLDER) {
    baseGameComboBox->addItem(QString::fromUtf8(gameType.first));
  }

  auto baseGameIndex = baseGameComboBox->findText(
      QString::fromUtf8(GameSettings(settings.Type()).FolderName()));
