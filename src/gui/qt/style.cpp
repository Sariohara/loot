...
std::optional<QString> loadStyleSheet(const QString& resourcePath) {
  ...
  if (!file.exists()) {
    ...
    if (logger) {
      logger->error("Unable to set stylesheet, {} not found.",
...
std::optional<QString> loadStyleSheet(
  ...
  auto builtInPath =
      QString(":/themes/%1.theme.qss").arg(QString::fromUtf8(themeName));
...
std::optional<QPalette> loadPalette(const QString& resourcePath) {
  ...
  if (activeButton && activeWindow) {
    ...
    palette = QPalette(QColor(QString::fromUtf8(*activeButton)),
                       QColor(QString::fromUtf8(*activeButton)));
  }
  ...
  for (const auto group : COLOR_GROUPS) {
    ...
    if (table) {
      for (const auto role : COLOR_ROLES) {
        ...
        if (roleValue) {
          palette.setColor(group, role, QColor(QString::fromUtf8(*roleValue)));
        }
      }
    }
  }
...
std::optional<QPalette> loadPalette(const std::filesystem::path& resourcesPath,
  ...
  auto builtInPath =
      QString(":/themes/%1.palette.toml").arg(QString::fromUtf8(themeName));
  ...
  if (logger) {
    logger->error("Failed to find the palette in the built-in resources.");
  }
